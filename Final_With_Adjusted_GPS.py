# Combined GPS + Hailo Object Detection with Adjusted GPS
...
# Camera FOV (degrees) - adjust to your camera specs
FOV_X = 70.0
FOV_Y = 50.0
...
class UserAppCallbackWithGPS(app_callback_class):
    ...
    def __init__(self, gps_obj: LatestGPS):
        ...
        # CSV header includes adjusted GPS
...
def app_callback(pad, info, user_data: UserAppCallbackWithGPS):
    ...
    # Draw bounding boxes and compute adjusted GPS
    lat_center, lon_center, alt, relative_alt, _, _, _, _ = user_data.gps_obj.get()
    ...
    if detections:
        for det in detections:
            ...
            # Adjusted GPS computation
            if lat_center is not None and relative_alt is not None:
                cx, cy = (x_min + x_max)/2, (y_min + y_max)/2
                dx_pix = cx - img_w/2
                dy_pix = cy - img_h/2
                fov_x_rad = math.radians(FOV_X)
                fov_y_rad = math.radians(FOV_Y)
                dx_m = dx_pix / img_w * 2 * relative_alt * math.tan(fov_x_rad/2)
                dy_m = dy_pix / img_h * 2 * relative_alt * math.tan(fov_y_rad/2)
                delta_lat = -dy_m / 111320.0
                delta_lon = dx_m / (111320.0 * math.cos(math.radians(lat_center)))
                obj_lat = lat_center + delta_lat
                obj_lon = lon_center + delta_lon
                gps_label = f"{det.get_label()}:{conf:.1f}%\nLat:{obj_lat:.7f}\nLon:{obj_lon:.7f}"
            else:
                gps_label = f"{det.get_label()}:{conf:.1f}%\nGPS:N/A"

            cv2.putText(frame_overlay, gps_label, (x_min, max(0, y_min - 5)),
    ...
    if lat_center is not None:
        gps_text = f"Lat: {lat_center:.7f}, Lon: {lon_center:.7f}, Alt: {alt:.1f}m, Rel Alt: {relative_alt:.1f}m"
    else:
        gps_text = "GPS: N/A"
